    ...
    <section id="projects" className="min-h-screen py-18 px-6 md:px-12 bg-background flex flex-col justify-center">
      <ScrollSection>
        <div className="flex flex-col md:flex-row justify-between items-end mb-16 border-b border-border pb-6">
           <h2 className="text-5xl md:text-7xl font-serif font-normal tracking-tight mb-4">Selected Works</h2>
           <span className="text-sm font-sans uppercase tracking-widest text-muted-foreground mb-4">03 / Projects</span>
        </div>
             ...
             <div key={i} className="group relative min-h-[300px] bg-muted/5 border border-border overflow-hidden cursor-pointer hover:bg-muted/10 transition-colors flex flex-col justify-between p-6">
                <div>
                   <div className="flex justify-between items-start mb-4">
                      <h3 className="text-2xl font-serif font-normal tracking-tight group-hover:opacity-70 transition-opacity">{project.title}</h3>
                      <span className="text-xs font-sans text-muted-foreground uppercase px-2 py-1 rounded-full border border-border">{project.year}</span>
                   </div>
                ...
                <div className="mt-auto">
                   <div className="text-xs font-sans uppercase tracking-wider text-muted-foreground mb-1">Tech Stack</div>
                   <div className="text-sm font-sans font-medium text-foreground">{project.tech}</div>
                </div>
             </div>
           ))}
        </div>
